...
if($status=="Inactive")
{
	$query_update="UPDATE ".$tblpref."expert set exp_status = 'Active'	WHERE exp_id='$id'";
...
if($status=="Active")
{
	$query_update="UPDATE ".$tblpref."expert set exp_status = 'Inactive' WHERE exp_id='$id'";
	if(!($result=mysql_query($query_update))){echo $query_update.mysql_error(); exit();}
	header("Location:index.php?flag=stat");
	exit;
}
...
if($mode=="del")
{
	$query="Delete from ".$tblpref."expert where exp_id='$id'";
	if(!($result=mysql_query($query))){echo mysql_error($query); exit;}

	$queryserv="Delete from ".$tblpref."expert_servies where exps_expert_id='$id'";
	if(!($resserv=mysql_query($queryserv))){echo mysql_error($queryserv); exit();}
	header("Location:index.php?flag=del");
	exit;
}
...
if($mode=="mail")
{
		...
		$queryserv="select * from ".$tblpref."services_category where cat_id='$exp_add[exp_service_area_id]'"; 
		if(!($resultser=mysql_query($queryserv))){ echo $queryserv.mysql_error(); exit;}
		$exp_ser=mysql_fetch_array($resultser);
		...
		$mess ="<html>
			...
			<TR>
				<TD align=left>Expert Type : ".$exp_ser[cat_name]. "</TD>
			</TR>
			<TR>
		...
		@mail($exp_add[exp_email],$sub,$mess,$mesheader);
